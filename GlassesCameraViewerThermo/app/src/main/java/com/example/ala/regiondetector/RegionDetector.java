...
public class RegionDetector implements EventListener {
    ...
    private Mat detectRegion(Mat mat, CascadeClassifier cascade, String region) {
        if (!mat.empty()) {
            ...
            if (objectArray.length > 0) {
                int index = findMaxRect(objectArray);
                Rect maxRect = new Rect(objectArray[index].tl(), objectArray[index].br());
                global_rect=maxRect;
                drawRect(mat, region, maxRect);
                Mat submat = mat.submat(maxRect);
                imageBuffer.add(submat);
            }
            else if (objectArray.length == 0 && global_rect.area()>0){
                drawRect(mat, region, global_rect);
                Mat submat = mat.submat(global_rect);
                imageBuffer.add(submat);
            }
    ...
    private int findMaxRect(Rect[] objectArray) {
        int index = 0;
        for (int i = 0; i < objectArray.length; i++) {
            if (objectArray[i].area() > objectArray[index].area()) {
                index = i;
            }
        }
        return index;
    ...
    private void drawRect(Mat mat, String region, Rect rect) {
        double avg;
